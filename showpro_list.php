...
$row = mysqli_fetch_row($query);
	...
	if ($pagenum > 1) {
        $previous = $pagenum - 1;
		$paginationCtrls .= '<a href="'.$_SERVER['PHP_SELF'].'?pn='.$previous.'" class="btn btn-info">&laquo;</a> &nbsp; &nbsp; ';
...
if ($pagenum != $last) {
$next = $pagenum + 1;
$paginationCtrls .= ' &nbsp; &nbsp; <a href="'.$_SERVER['PHP_SELF'].'?pn='.$next.'" class="btn btn-info">&raquo;</a> ';
}
	}

$count =mysqli_num_rows($numquery);
...
<section class="box_shop p-md-5">
    ...
    <h3 class="text-center"style="margin:100px 0 50px 0;"><i class="fas fa-times text-danger"></i> ไม่พบข้อมูลโปรโมชั่นหอพัก <i class="fas fa-times text-danger"></i></h3>
...
<!-- script animation AOS -->

<script>
AOS.init();
</script>

<!--end  script animation AOS -->